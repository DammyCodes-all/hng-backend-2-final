...
import { fetchCountryData, getExchangeRates, calculateGDP, generateSummaryImage, } from "./lib/utils.js";
    ...
    try {
        ...
        try {
            ...
            const topCountries = await db
                ...
                .limit(5);
            // Generate the image
            const imagePath = await generateSummaryImage(countryList.length, topCountries, now);
...
app.get("/countries", async (req, res) => {
    try {
        ...
        if (conditions.length > 0) {
            query = query.where(conditions.reduce((acc, condition) => sql `${acc} AND ${condition}`));
        }
        // Apply sorting
        if (sort && typeof sort === "string") {
            switch (sort.toLowerCase()) {
                case "gdp_desc":
                    query = query.orderBy(desc(countries.estimatedGdp));
                    break;
                case "gdp_asc":
                    query = query.orderBy(asc(countries.estimatedGdp));
                    break;
                case "population_desc":
                    query = query.orderBy(desc(countries.population));
                    break;
                case "population_asc":
                    query = query.orderBy(asc(countries.population));
                    break;
                case "name_asc":
                    query = query.orderBy(asc(countries.name));
                    break;
                case "name_desc":
                    query = query.orderBy(desc(countries.name));
                    break;
                default:
